...
public class GatewayConfig {
    ...
    public RouteLocator routes(RouteLocatorBuilder builder){
        return builder.routes()
                ...
                .route("demo-service1-users" , r -> r.path("/api/users/**").filters(f -> f.filter(filter)).uri("lb://demo-service1"))
                .route("demo-service1-en1" , r -> r.path("/api/en1/**").filters(f -> f.filter(filter)).uri("lb://demo-service1"))
